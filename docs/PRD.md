...
- **Cấu hình mẫu in** (bổ sung 2026-08-05, theo yêu cầu người dùng — hoàn thành, bắt đầu với phiếu xuất kho; **viết lại hoàn toàn 2026-08-19** theo phản hồi người dùng): trang "Mẫu in" (menu Cấu hình, thuộc quyền `cau_hinh`) liệt kê các **loại phiếu** đã có mẫu trong hệ thống (mỗi loại phiếu cần có sẵn 1 trang render tương ứng trong code mới dùng được, không phải mẫu người dùng tự tạo/đặt tên tùy ý), bấm vào để mở trình chỉnh sửa. Mỗi loại phiếu chỉ có **đúng 1 mẫu, sửa trực tiếp** — không có khái niệm nhiều mẫu/loại + chọn "đang dùng".
  - **Từ 2026-08-19**: mẫu là **1 khung HTML/CSS thật do người dùng tự soạn** (ô soạn mã, không phải trình soạn WYSIWYG kéo-thả) — cho định vị/bố cục tự do hoàn toàn bằng CSS thật, mạnh hơn hẳn cách bật/tắt cột cố định trước đây. Hệ thống chỉ lo: thay **token** dạng `{{TenBien}}` (danh sách cố định theo từng loại phiếu, có panel tham chiếu bấm-để-chèn cạnh ô soạn) bằng dữ liệu thật lúc in, và **lặp dòng bảng sản phẩm** theo khối đánh dấu `<!-- items:start -->...<!-- items:end -->` (token trong khối này dùng tiền tố `Item.`, vd `{{Item.ProductName}}`). Có thể ẩn cả 1 đoạn nếu dữ liệu rỗng bằng khối `<!-- if:TenBien -->...<!-- endif -->`. Không còn khái niệm "cột bảng sản phẩm"/"Số tiền viết bằng chữ" cấu hình riêng — tất cả là token bình thường đặt vị trí tự do.
  - Xem trước trực quan (dữ liệu mẫu) trong khung cô lập hoàn toàn CSS với trang quản trị trước khi lưu; lưu bị chặn nếu mẫu chứa token gõ sai tên (báo rõ danh sách token không hợp lệ). Chọn khổ giấy A4 dọc/ngang.
...
### 4.10 Bảo hành (bổ sung 2026-08-01, theo yêu cầu người dùng; **mở rộng 2026-08-19**: quy về theo dự án + lịch sử bảo hành)
...
- Mỗi bản ghi bảo hành gồm: khách hàng, **dự án (tùy chọn — xem bên dưới)**, số điện thoại, địa chỉ, ngày nghiệm thu, ngày hết hạn, thời gian bảo hành, ghi chú.
...
- Vô hiệu hóa/mở lại: mọi người có quyền module `bao_hanh` (module riêng từ 2026-08-08 — trước đó dùng chung `cong_no`, xem `docs/DECISIONS.md`). **Xóa cứng chỉ Admin**, và chỉ xóa được khi bản ghi **chưa có lịch sử bảo hành nào** — khác các đối tượng khác trong hệ thống vốn thường cho phép xóa nếu chưa có lịch sử, ở đây thêm điều kiện Admin theo đúng yêu cầu ban đầu.
- **Giao diện chi tiết khách hàng** (trang `customer-detail.html`): hiển thị thông tin cơ bản khách hàng + toàn bộ bản ghi bảo hành của khách hàng đó dưới dạng **Card**, mỗi card hiện "Số ngày còn lại" (số lớn, đổi màu theo mức độ khẩn cấp: còn nhiều — xanh, ≤30 ngày — cam, đã hết hạn — đỏ) và "Ngày hết hạn" cụ thể, lấy trực tiếp từ dữ liệu bảo hành (không lưu số ngày còn lại cố định — luôn tính lại từ ngày hết hạn tại thời điểm xem, đúng nguyên tắc không lưu giá trị suy ra được của dự án).

**Gắn bảo hành với Dự án + Lịch sử bảo hành (bổ sung 2026-08-19, theo yêu cầu người dùng)**:
- Trên popup Thêm/Sửa bảo hành, sau khi chọn Khách hàng có thêm ô **"Dự án" (không bắt buộc)** — chỉ liệt kê dự án CỦA ĐÚNG khách hàng đang chọn (lọc qua `GET /projects?partner_id=`). Không bắt buộc chọn — khách hàng chưa dùng module Dự án hoặc chưa có dự án nào vẫn tạo bảo hành bình thường như trước. 1 dự án có thể có nhiều bản ghi bảo hành (ví dụ nhiều hạng mục/đợt nghiệm thu khác ngày).
- Trang "Chi tiết dự án" (`project-detail.html`) có thêm **tab "Bảo hành"** (chỉ hiện với tài khoản có cả quyền `du_an` và `bao_hanh`) — liệt kê mọi bản ghi bảo hành đã gắn với dự án đó, kèm bảng lịch sử bảo hành ngay bên dưới mỗi bản ghi. Tab này chỉ HIỂN THỊ + quản lý lịch sử bảo hành — tạo/sửa bản ghi bảo hành gốc vẫn thực hiện tại trang "Bảo hành" (có nút "+ Thêm bảo hành" điều hướng sang, chọn sẵn đúng khách hàng + dự án).
- **Lịch sử bảo hành**: mỗi bản ghi bảo hành có thể ghi nhận nhiều lần bảo hành/bảo trì thực tế, quản lý được cả từ trang "Bảo hành" (nút "Xem chi tiết") lẫn tab "Bảo hành" của dự án — 2 nơi cùng đọc/ghi chung 1 dữ liệu. Mỗi lần bảo hành gồm: **Lần** (tự động đánh số tăng dần theo đúng thứ tự tạo, không nhập tay), Ngày thực hiện, Nội dung bảo hành, Nhân viên thực hiện (chọn từ toàn bộ tài khoản người dùng đang hoạt động trong hệ thống, không giới hạn theo người tham gia dự án), Kết quả (Hoàn thành/Chưa hoàn thành/Tạm dừng), Ghi chú. Sửa/xóa được từng lần (không giữ nguyên tắc "chỉ điều chỉnh bù trừ" như Kho/Công nợ — đây là nhật ký vận hành, không phải sổ cái tài chính).
