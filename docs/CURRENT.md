...
- ✅ **Bảo hành (2026-08-01, ngoài phase, theo yêu cầu người dùng)**: module mới thuộc menu Khách hàng — migration `017` (bảng `warranties`, chỉ gắn khách hàng), CRUD đầy đủ (`warranties.routes.js`), trang danh sách (`warranties.html`) + trang dùng chung cho thêm mới/xem/sửa (`warranty-detail.html`, lưu thay đổi ngay trên trang, không dùng modal), tương tác 2 chiều Thời gian bảo hành ↔ Ngày hết hạn (`warranty-calc.js`). Trang mới `customer-detail.html` (trước đây chưa có) hiển thị card Bảo hành (số ngày còn lại + ngày hết hạn). Xóa cứng chỉ Admin. Nhân tiện sửa thêm 3 lỗi CSS `[hidden]` bị ghi đè phát hiện khi test (`.empty-state`, `.page-header-actions`, `.page-header-actions .btn-secondary`).
