...
## Trang chi tiết dạng tab (project-detail.html) — pattern MỚI, khác mọi trang chi tiết trước đó

> Bổ sung 2026-08-04 khi làm module "Quản lý dự án" (Đợt 1). Mọi trang chi tiết trước đây (`product-detail.html`, `customer-detail.html`...) đều cuộn dọc 1 mạch — đây là trang chi tiết đầu tiên cần chia nhiều khu vực nội dung lớn (Tổng quan, Giai đoạn, và sẽ có thêm Công việc/Vật tư/Thanh toán/Phát sinh ở các đợt sau) nên tách thành tab thay vì cuộn dài.

- `.detail-tabs`: thanh ngang đặt ngay dưới `.page-header`, chứa các nút `.detail-tab` (không phải thẻ `<a>`, JS toggle thuộc tính `[hidden]` trên `.tab-panel` tương ứng — không dùng thư viện tab ngoài). Trạng thái active: chữ màu primary + đậm + viền dưới 2px màu primary; các tab khác chữ màu muted, đổi màu khi hover.
- Mỗi khu vực nội dung là 1 `.tab-panel` con trực tiếp của `<main>`, ẩn/hiện bằng `[hidden]` — **không** dùng `display:none` viết tay (tránh lặp lại đúng loại lỗi `[hidden]` bị đè đã gặp nhiều lần trước đây, xem mục "Bảo hành" bên dưới).
- Tab "Tổng quan" tái dùng nguyên `.stat-grid`/`.stat-card` (số liệu nổi bật) + `.detail-info-grid` (thông tin dạng label-value) đã có sẵn — không tạo layout riêng.
- **Trạng thái dự án (5 giá trị: Chuẩn bị/Đang thực hiện/Tạm dừng/Hoàn thành/Hủy)** dùng badge riêng `.project-status-badge--<status>`, **không** dùng lại `.badge-active`/`.badge-inactive` vì đó chỉ có 2 trạng thái nhị phân.
- **Danh sách người tham gia dự án** (modal thêm/sửa dự án + tab Tổng quan chỉ đọc): `.member-picker-row` (select chọn tài khoản + input vai trò tự do + nút "+ Thêm") và `.member-list`/`.member-row` cho kết quả. **Không** dùng lại `.repeatable-row` (input `flex:1` chia đều sai tỷ lệ khi có 2 trường khác loại — select tên cần rộng hơn input vai trò).

## Biểu đồ Gantt (tab "Giai đoạn" của project-detail.html) — SVG tự vẽ tay

> Bổ sung 2026-08-04. Nhất quán với quyết định đã chốt ở trang Báo cáo (2026-08-01): không dùng Chart.js/thư viện biểu đồ ngoài nào, tự vẽ SVG bằng JS (`renderGanttChart()` trong `project-detail.js`).

- Mỗi giai đoạn 1 dòng ngang cố định (`.gantt-card` bọc `<svg class="gantt-svg">`): nhãn tên giai đoạn bên trái (cột rộng cố định), thanh ngang (`<rect rx>`) thể hiện khoảng `planned_start`–`planned_end`, màu theo trạng thái (`chua_bat_dau` xám nhạt, `dang_lam` primary, `hoan_thanh` accent). Giai đoạn chưa có ngày hiện chữ nghiêng "Chưa đặt lịch" thay vì thanh trống — không vẽ thanh giả/thanh 0 chiều dài.
- **Bài học phát hiện khi test qua trình duyệt thật (quan trọng, tránh lặp lại)**: nếu trục thời gian tính đúng bằng khoảng `[minDate, maxDate]` của chính dữ liệu (không đệm thêm), giai đoạn có ngày bắt đầu trùng đúng `minDate` sẽ nằm sát mép trái biểu đồ và **không có nhãn tháng nào hiện ra trước đó** để biết thanh đó thuộc tháng nào (tháng chứa `minDate` bị coi là "trước vùng vẽ" nên bị bỏ qua). Đã sửa bằng cách đệm thêm 3 ngày mỗi bên trước khi tính tỷ lệ, đồng thời ép vị trí nhãn/đường kẻ tháng đầu tiên tối thiểu bằng đúng mép trái vùng vẽ (không bỏ qua) — đảm bảo luôn có ít nhất 1 nhãn tháng làm mốc tham chiếu dù thanh đầu tiên nằm sát mép nào.
- **Bài học thứ 2 (2026-08-04, Đợt 2, phát sinh từ chính cách sửa ở trên)**: đệm thêm 3 ngày đôi khi đẩy domain sang lọt qua đầu tháng liền trước chỉ vài ngày — khi đó tháng đó **và** tháng kế tiếp đều bị ép vị trí về gần sát mép trái (cùng do rule "tối thiểu bằng mép trái" ở trên), khiến 2 nhãn chữ đè lên nhau không đọc được (vd "Th6/2026" chồng "Th7/2026"). Đã sửa bằng cách theo dõi vị trí nhãn tháng vẽ gần nhất, bỏ qua vẽ nhãn (chữ) nào cách nhãn liền trước dưới 55px — **vẫn giữ đường kẻ phân tháng** (chỉ bỏ chữ, không bỏ mốc chia). Rút ra nguyên tắc chung: mọi nhãn chữ đặt theo tọa độ tính toán (không phải lưới cố định đều nhau) đều cần kiểm tra khoảng cách tối thiểu với nhãn liền trước trước khi vẽ.
- **Phần tô đậm bên trong thanh = % hoàn thành** (Đợt 2, tính từ tỷ lệ công việc `hoan_thanh`/tổng công việc của giai đoạn, `null` nếu giai đoạn chưa có việc nào — không vẽ phần tô đậm khi đó): vẽ 2 `<rect>` chồng nhau cùng `x`/`rx` — rect nền `fill-opacity="0.35"` full chiều dài thanh, rect tô đậm phía trên chỉ rộng bằng đúng tỷ lệ %. **Nhãn số % đặt NGAY SAU thanh** (không đặt đè lên thanh) — tránh phải tính màu chữ tương phản thay đổi theo vị trí đè lên phần tô đậm hay phần nhạt của chính thanh đó.
- Không ép chiều rộng cố định — `chartWidth` tính từ `clientWidth` của `.gantt-card` (tối thiểu 640px, có `overflow-x:auto` cho màn hẹp), tự vẽ lại khi resize cửa sổ.

## Dòng giai đoạn bấm mở rộng bảng công việc con (project-detail.html, tab "Giai đoạn")

> Bổ sung 2026-08-04, theo yêu cầu người dùng: bỏ hẳn tab "Công việc" riêng, gộp toàn bộ CRUD công việc vào ngay tab "Giai đoạn" — bấm vào 1 dòng giai đoạn (trong bảng "Danh sách giai đoạn") sẽ xổ ra 1 dòng `<tr>` mới ngay bên dưới chứa bảng công việc con của đúng giai đoạn đó.

- `.phase-row` (`cursor:pointer`, click vào bất kỳ đâu trong dòng trừ vùng nút Sửa/Xóa sẽ toggle mở/đóng) + `.row-toggle-icon` (chevron lên/xuống đặt trước số thứ tự, báo hiệu trạng thái mở rộng).
- Dòng mở rộng `.phase-tasks-row` (`<td colspan>` bằng đúng số cột bảng cha) nền `--color-muted` để phân biệt rõ với dòng giai đoạn — bên trong là `.subtask-panel` chứa tiêu đề + nút "+ Thêm công việc" (`data-action="add-task"`, mở modal công việc với giai đoạn preset sẵn) và 1 bảng `.subtask-table` (tái dùng `.data-table`, chỉ giảm padding/font-size).
- Mỗi dòng công việc trong `.subtask-table` có sẵn 2 ô `<input type="date">` (Bắt đầu thực tế/Kết thúc thực tế — **nhập tay trực tiếp**, không cần mở modal), 1 `<select>` Trạng thái, và nút "Lưu" (`.btn-sm`, PUT ngay lập tức) — cho phép cập nhật nhanh mà không rời khỏi bảng. Icon "Sửa" cạnh đó vẫn mở modal đầy đủ khi cần đổi tên/người phụ trách/giai đoạn/ngày kế hoạch.
- **`.btn-sm`**: biến thể nhỏ gọn dùng chung cho `.btn-add`/`.btn-secondary` khi đặt trong không gian chật (dòng bảng, panel mở rộng) — chỉ giảm padding/font-size + bỏ `margin-top` mặc định của `.btn-secondary`, không đổi màu/kiểu.
- Biểu đồ Gantt dùng chung 1 trạng thái mở rộng (`expandedPhaseIds`, biến JS cấp module) với bảng — bấm vào giai đoạn trên Gantt (bọc cả nhãn tên + thanh trong 1 `<g data-phase-id>` để nhận click) cũng xổ ra danh sách công việc **dạng text ngắn gọn** ngay dưới thanh đó (chấm tròn màu theo trạng thái + tên + trạng thái + "Trễ X ngày" nếu có) — người dùng chọn phương án này thay vì vẽ thêm thanh SVG riêng cho từng công việc, giữ Gantt đơn giản. Chiều cao mỗi dòng Gantt vì vậy không còn cố định — tính động qua `computePhaseRowHeight()` (cộng thêm số dòng công việc × chiều cao 1 dòng text khi đang mở rộng).
- Giai đoạn chưa có công việc nào hiện `"Chưa có công việc nào"` (chữ nghiêng, màu muted) ở cả 2 nơi — không để trống trơn.

**Bài học quan trọng phát hiện khi sửa lỗi hiển thị (2026-08-04, người dùng báo "nền bị trắng" 2 lần liên tiếp) — cần nhớ cho mọi bảng động khác sau này:**
1. **Tuyệt đối không đặt `display:flex` trực tiếp lên `<td>`** khi bảng đang `table-layout:auto` (mặc định của `.data-table`) — trình duyệt tính sai độ rộng cột tự nhiên của ô đó, đặc biệt khi kết hợp với `.data-table td:last-child{width:1%}` (dùng để ép cột hành động hẹp lại). Thực tế đã đo được ô chỉ được cấp ~24px thay vì ~150px cần cho 3 nút, khiến nút tràn ra ngoài ô đè lên nền bên cạnh — nhìn như "mất viền/mất nền" dù nút vẫn style đúng. Cách đúng: **giữ nguyên `<td>` ở display mặc định, bọc nội dung cần flex trong 1 `<div>` riêng bên trong ô** (`.subtask-actions` là `<div>`, không phải `<td>`).
